...
void print_vector(const char* label, float x, float y)
{
    printf("%s: (%.2f, %.2f)\n", label, x, y);
}
...
int main(int argc, char* argv[])
{
    ...
    printf("\n\n");
    printf("========================================\n");
    printf("=== MATRIX STACK - UNDOABLE OPERATIONS ===\n");
    printf("========================================\n");
    
    MatrixStack stack;
    init_matrix_stack(&stack);
    
    float test_point[3] = {2.0f, 2.0f, 1.0f};
    float test_result[3];
    float test_matrix[3][3];
    
    init_identity_matrix(test_matrix);
    
    printf("\n[INITIAL STATE]\n");
    printf("Current transformation matrix:\n");
    print_matrix(test_matrix);
    transform_point(test_matrix, test_point, test_result);
    print_vector("Point after transformation", test_result[0], test_result[1]);
    
    printf("\n--- OPERATION 1: Scale by (2, 2) ---\n");
    printf("Saving current state to stack...\n");
    push_matrix(&stack, test_matrix);
    printf("Stack size: %d\n", stack_size(&stack));
    scale(test_matrix, 2.0f, 2.0f);
    printf("New transformation matrix:\n");
    print_matrix(test_matrix);
    transform_point(test_matrix, test_point, test_result);
    print_vector("Point after transformation", test_result[0], test_result[1]);
    
    printf("\n--- OPERATION 2: Rotate by 45 degrees ---\n");
    printf("Saving current state to stack...\n");
    push_matrix(&stack, test_matrix);
    printf("Stack size: %d\n", stack_size(&stack));
    rotate(test_matrix, M_PI / 4.0f);
    printf("New transformation matrix:\n");
    print_matrix(test_matrix);
    transform_point(test_matrix, test_point, test_result);
    print_vector("Point after transformation", test_result[0], test_result[1]);
    
    printf("\n--- OPERATION 3: Translate by (3, 3) ---\n");
    printf("Saving current state to stack...\n");
    push_matrix(&stack, test_matrix);
    printf("Stack size: %d\n", stack_size(&stack));
    shift(test_matrix, 3.0f, 3.0f);
    printf("New transformation matrix:\n");
    print_matrix(test_matrix);
    transform_point(test_matrix, test_point, test_result);
    print_vector("Point after transformation", test_result[0], test_result[1]);
    
    printf("\n--- UNDO: Going back to state before translation ---\n");
    if (pop_matrix(&stack, test_matrix)) {
        printf("Restored state from stack!\n");
        printf("Stack size: %d\n", stack_size(&stack));
    ...
    printf("\n--- UNDO: Going back to state before rotation ---\n");
    if (pop_matrix(&stack, test_matrix)) {
        printf("Restored state from stack!\n");
        printf("Stack size: %d\n", stack_size(&stack));
        printf("Transformation matrix:\n");
    ...
    printf("\n--- UNDO: Going back to initial state ---\n");
    if (pop_matrix(&stack, test_matrix)) {
        printf("Restored state from stack!\n");
        printf("Stack size: %d\n", stack_size(&stack));
        printf("Transformation matrix:\n");
    ...
    printf("\n--- Trying to undo when stack is empty ---\n");
    if (pop_matrix(&stack, test_matrix)) {
        printf("Restored state from stack!\n");
    } else {
        printf("ERROR: Cannot undo - stack is empty!\n");
        printf("Stack is empty: %s\n", is_stack_empty(&stack) ? "YES" : "NO");
    }
